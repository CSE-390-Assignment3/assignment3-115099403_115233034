...
* Timeout for every thread is custom set to 100ms* maxsteps (a safe high value - giving it enough time to execute) 
* There is a single monitor thread that keeps checking if a thread has timed out or not, if it has timed out, it reduces the priority to idle, dumps the output and spawns another thread in its place.
